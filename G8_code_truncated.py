...
with open("/kaggle/input/qesselanbya/prophet_youssef.txt", "r", encoding="utf-8") as f:
    raw_text = f.read()
...
# 8. Interactive interface with truncated display
def interactive_rag():
    ...
    while True:
        ...
        # BM25 results (truncated)
        print("\n>>> نتائج BM25 :")
        for score, txt in search_bm25(q, 5):
            display = txt[:100] + ('...' if len(txt) > 100 else '')
            print(f"({score:.2f}) {display}\n")

        # Semantic results (truncated)
